...
minimize Objective: sum{g in genre, s in song} ( -Actual[s,g] * log( exp(n[s,g]) / (sum{f in feature} exp(n[s,f]) ) ) ) + (0.5 * sum{g in genre, p in part, f in feature} w[g,p,f]^2) +  (100 * sum{f in feature} z[f]);

s.t. ct1 {g in genre, s in song}: n[s,g] == (sum{p in part, f in feature} (w[g,p,f] * X[s,p,f]) ) + b[g];

s.t. ct2 {g in genre, p in part, f in feature}: w[g,p,f] >= -M * z[f];

s.t. ct3 {g in genre, p in part, f in feature}: w[g,p,f] <= M * z[f];
